...
fn random_neuron_with_stake(hotkey:u64, stake_to_init: u64, ip:u128, port:u16, ip_type:u8, modality: u8, coldkey:u64) -> NeuronMetadata<u64> {
    let _ = Subtensor::subscribe(<<Test as system::Config>::Origin>::signed(hotkey), ip, port, ip_type, modality, coldkey);
    let neuron = Subtensor::get_neuron_for_hotkey(&hotkey);

    // Let's give this neuron an initial stake.
    Subtensor::add_stake_to_neuron_hotkey_account(neuron.uid, stake_to_init); // Add the stake.
    assert_eq!(stake_to_init, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid)); // Check that the stake is there.
    neuron
}
...
fn test_emit_does_not_exist() {
	new_test_ext().execute_with(|| {
        ...
        // Let's call an emit.
        let total_emission:u64 = Subtensor::emit_for_neuron(&neuron);
...
fn test_multiemit_per_block() {
	new_test_ext().execute_with(|| {
        ...
        assert_ok!(Subtensor::set_weights(<<Test as Config>::Origin>::signed(hotkey), weight_uids.clone(), weight_values.clone()));
        assert_eq!(Subtensor::get_weights_for_neuron(&neuron), (weight_uids, weight_values)); // Check the weights are set.

        // Let's call an emit.
        let total_emission:u64 = Subtensor::emit_for_neuron(&neuron);
        assert_eq!(total_emission, 0);
        assert_eq!(1000000000, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid)); // Check that the stake is there.
...
fn test_emission_to_other() {
	new_test_ext().execute_with(|| {
		...
		let weight_values = vec![u32::MAX]; 
        assert_ok!(Subtensor::set_weights(<<Test as Config>::Origin>::signed(hotkey_one), weight_uids.clone(), weight_values.clone()));
        assert_eq!(Subtensor::get_weights_for_neuron(&neuron_one), (weight_uids, weight_values)); // Check the weights are set.

        // Let's call an emit at block 1
        let total_emission:u64 = Subtensor::emit_for_neuron(&neuron_one);
        assert_eq!(total_emission, 0);
        assert_eq!(0, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron_two.uid)); // Check that the stake is there.
        ...
        assert_eq!(500000000, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron_two.uid)); // Check that the stake is there.
        ...
        run_to_block(3);
         ...
         assert_eq!(1166666666, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron_two.uid)); // Check that the stake is there.
	});
}
...
fn test_self_weight() {
	new_test_ext().execute_with(|| {
        ...
        assert_eq!(1_500_000_000, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid)); // Check that the stake is there.
        ...
        assert_eq!(2_000_000_000, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid)); // Check that the stake is there.
...
fn test_many_with_weights() {
	...
	for (i, neuron) in neurons.iter().enumerate() {
	        Subtensor::add_stake_to_neuron_hotkey_account(neuron.uid, stakes[i]);
        }
        for (i, neuron) in neurons.iter().enumerate() {
		assert_ok!(Subtensor::set_weights(<<Test as Config>::Origin>::signed(neuron.uid), weight_uids[i].clone(), weight_vals[i].clone()));
        }
        let mut emission_per_neuron = vec![];
        for (_, neuron) in neurons.iter().enumerate() {
		emission_per_neuron.push(Subtensor::emit_for_neuron(&neuron));
        }
        for (i, _) in neurons.iter().enumerate() {
            assert_eq!(emission_per_neuron[i], 0);
        }
        for (i, neuron) in neurons.iter().enumerate(){
	        assert_eq!(stakes[i], Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid));
        }
        run_to_block(2 * n);
        let mut emission_per_neuron = vec![];
        for (_, neuron) in neurons.iter().enumerate() {
        ...
        for (i, neuron) in neurons.iter().enumerate(){
			assert!( close( stakes[i] + 1000000000, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid), 100 ));
        }
...
fn test_emission_after_many_blocks_one_edge() {
	new_test_ext().execute_with(|| {
        ...
        Subtensor::add_stake_to_neuron_hotkey_account(neuron_a.uid, 1_000_000_000);
        Subtensor::add_stake_to_neuron_hotkey_account(neuron_b.uid, 1_000_000_000);

        assert_eq!(Subtensor::get_total_stake(), 2_000_000_000);

        Subtensor::set_new_weights(&neuron_a, &vec![neuron_b.uid], &vec![u32::MAX]);
        Subtensor::set_new_weights(&neuron_b, &vec![neuron_a.uid], &vec![u32::MAX]);
        ...
        assert_eq!(Subtensor::get_pending_emission_for_neuron(neuron_a.uid), 250_000_000_000 as u64);
        assert_eq!(Subtensor::get_pending_emission_for_neuron(neuron_b.uid), 250_000_000_000 as u64);

        Subtensor::emit_for_neuron(&neuron_a); // This should transfer the pending emission to neuron_b
        Subtensor::emit_for_neuron(&neuron_b); // This should transfer the pending emission to neuron_a

        // neurons a&b should have 250 + 1 (initial) * 10 ^ 9 stake
        assert_eq!(Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron_a.uid), 251_000_000_000 as u64);
        assert_eq!(Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron_b.uid), 251_000_000_000 as u64);
	});
}
	...
	new_test_ext().execute_with(|| {
        ...
        for i in 1..(blocks_to_run + 1) {
                ...
                for neuron in neurons.iter() {
                        Subtensor::emit_for_neuron(&neuron);
                }            
        }
        for neuron in neurons.iter() {
                Subtensor::emit_for_neuron(&neuron);
        ...
        for neuron in neurons.iter() {
                sum_of_stake += Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid);
        }
...
fn test_can_emission_proceed_ok() {
	new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::can_emission_proceed(&emission, &weight_uids, &weight_vals), true)
	});
}
...
fn test_can_emission_proceed_fails_no_emission() {
	new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::can_emission_proceed(&emission, &weight_uids, &weight_vals), false)
...
fn test_calculate_stake_increment_ok() {
	new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::calculate_stake_increment(emission, weight), 1);
	});
}
...
fn test_get_pending_emission_for_neuron_ok() {
	new_test_ext().execute_with(|| {
        ...
        // Set it up
        Subtensor::update_pending_emission_for_neuron(uid, pending_emission);
        assert_eq!(Subtensor::get_pending_emission_for_neuron(uid), pending_emission);
	});
}
...
fn test_get_pending_emission_for_neuron_uid_does_not_exist() {
	new_test_ext().execute_with(|| {
        let uid = 55;
        assert_eq!(Subtensor::get_pending_emission_for_neuron(uid), 0);
	});
}
...
fn test_reset_pending_emission_for_neuron_ok() {
	new_test_ext().execute_with(|| {
        ...
        Subtensor::update_pending_emission_for_neuron(uid, emission);
        Subtensor::reset_pending_emission_for_neuron(uid);
        assert_eq!(Subtensor::get_pending_emission_for_neuron(uid), 0);
	});
}
...
fn test_update_last_emit_for_neuron_ok() {
	new_test_ext().execute_with(|| {
        ...
        for x in 0..10 {
            run_to_block(x);
            Subtensor::update_last_emit_for_neuron(uid);
            run_to_block(x+1);
            assert_eq!(Subtensor::get_last_emit_for_neuron(uid), x);
        }
	});
}
...
fn test_get_last_emit_for_neuron_ok() {
	new_test_ext().execute_with(|| {
        ...
        Subtensor::update_last_emit_for_neuron(uid);
        assert_eq!(Subtensor::get_last_emit_for_neuron(uid), 10);
	});
}
...
fn test_get_last_emit_for_neuron_default() {
	new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::get_last_emit_for_neuron(uid), 0)
	});
}
...
fn test_calculate_new_emission_ok() {
	new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::calculate_new_emission(block_reward, stake_fraction), 20);
	});
}
...
fn test_calculate_new_emission_stake_out_of_bound() {
	new_test_ext().execute_with(|| {
        ...
        Subtensor::calculate_new_emission(block_reward, stake_fraction);
...
fn test_update_pending_emission_for_neuron_ok() {
	new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::get_pending_emission_for_neuron(uid), 0); // Default == 0
        Subtensor::update_pending_emission_for_neuron(uid, increment_1);
        assert_eq!(Subtensor::get_pending_emission_for_neuron(uid), 5);

        Subtensor::update_pending_emission_for_neuron(uid, increment_2);
        assert_eq!(Subtensor::get_pending_emission_for_neuron(uid), 15);

        Subtensor::update_pending_emission_for_neuron(uid, increment_3);
        assert_eq!(Subtensor::get_pending_emission_for_neuron(uid), 30);
	});
}
