...
fn fee_from_emission_priority_with_neuron_and_weights() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Subtensor::set_weights(Origin::signed(hotkey_account_id), weight_uids.clone(), weight_values.clone()));
...
fn fee_from_emission_priority_with_neuron_and_weights_and_stake() {
    new_test_ext().execute_with(|| {
        ...
        let weight_values = vec![u32::MAX];
        assert_ok!(Subtensor::set_weights(Origin::signed(hotkey_account_id), weight_uids.clone(), weight_values.clone()));
        Subtensor::add_stake_to_neuron_hotkey_account(neuron.uid, 100000000); // Add the stake.
...
fn fee_from_emission_priority_with_neuron_and_weights_and_stake_and_run_to_block() {
    new_test_ext().execute_with(|| {
        ...
        let weight_values = vec![u32::MAX];
        assert_ok!(Subtensor::set_weights(Origin::signed(hotkey_account_id), weight_uids.clone(), weight_values.clone()));
        Subtensor::add_stake_to_neuron_hotkey_account(neuron.uid, 1_000_000_000); // Add the stake.
        ...
        assert_eq!(1_000_000_000, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid)); // Check that his stake has increased with 99% of the BR
        let _total_emission: u64 = Subtensor::emit_for_neuron(&neuron); // actually do the emission.
    });
}
...
fn test_emission_low_priority_but_emission_goes_to_user() {
    new_test_ext().execute_with(|| {
        ...
        let weight_values = vec![u32::MAX];
        assert_ok!(Subtensor::set_weights(Origin::signed(hotkey_account_id_2), weight_uids.clone(), weight_values.clone()));
        Subtensor::add_stake_to_neuron_hotkey_account(neuron_2.uid, 100000000); // Add the stake.
        ...
        run_to_block(1);
        assert_eq!(ChargeTransactionPayment::<Test>(PhantomData).validate(&hotkey_account_id_2, &call, &info, len).unwrap().priority, 0);
        let _total_emission: u64 = Subtensor::emit_for_neuron(&neuron_2); // actually do the emission.
        assert_eq!(500000000, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron_3.uid));
    });
}
...
fn fee_from_emission_priority_with_neuron_and_adam() {
    new_test_ext().execute_with(|| {
        ...
        run_to_block(1);
        assert_eq!(ChargeTransactionPayment::<Test>(PhantomData).validate(&hotkey_account_id, &call, &info, len).unwrap().priority, 500_000);
        assert_eq!(1_000_000_000, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid)); // Check that his stake has not increased.

        let _total_emission: u64 = Subtensor::emit_for_neuron(&neuron); // actually do the emission.
        assert_eq!(1_500_000_000, Subtensor::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid)); // Check that his stake has increased (he is *not* adam)
    });
}
...
fn test_charge_transaction_payment_can_pay_remove_stake_ok_enough_stake() {
    ...
    new_test_ext().execute_with(|| {
        ...
        let _ = Subtensor::add_stake_to_neuron_hotkey_account(adam.uid, 100_000);
        let result = ChargeTransactionPayment::<Test>::can_pay_remove_stake(&coldkey_id, &hotkey_id, len);
        assert_eq!(result, Ok(20000));
    });
}
...
fn test_charge_transaction_payment_can_pay_remove_stake_err_insufficient_funds() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::get_stake_of_neuron_hotkey_account_by_uid(adam.uid), 0);
...
fn pre_dispatch_works() {
    new_test_ext().execute_with(|| {
        let hotkey_account_id = 1;
        let neuron = subscribe_neuron(hotkey_account_id, 10, 666, 4, 0, 66);
        assert_ok!(Subtensor::set_weights(Origin::signed(hotkey_account_id), vec![neuron.uid], vec![u32::MAX]));
        Subtensor::add_stake_to_neuron_hotkey_account(neuron.uid, 1000000000); // Add the stake.
        let call = SubtensorCall::set_weights(vec![0], vec![0]).into();
        let info = DispatchInfo::default();
        let len = 10;
...
fn test_post_dispatch_does_not_deposit_to_adam_on_error() {
    new_test_ext().execute_with(|| {
        ...
        let result = Subtensor::get_stake_of_neuron_hotkey_account_by_uid(adam_id);
        assert_eq!(result, 0);
        ...
        assert_eq!(Subtensor::get_stake_of_neuron_hotkey_account_by_uid(adam_id), 0);
    });
}
...
fn post_dispatch_deposit_to_transaction_fee_pool_works() {
    new_test_ext().execute_with(|| {
        ...
        let pre = ChargeTransactionPayment::<Test>(PhantomData).pre_dispatch(&hotkey_account_id, &call, &info, len).unwrap();
        assert!(ChargeTransactionPayment::<Test>::post_dispatch(pre, &info, &PostDispatchInfo { actual_weight: Some(0), pays_fee: Default::default() }, len, &Ok(())).is_ok());
        assert_eq!(Subtensor::get_transaction_fee_pool(), 5_000_000); // 1%
    });
}
...
/************************************************************
    ...
    TODO (shibshib): Fix these tests so they work with the new mocked
    Substrate 3.0 and Frame v2.
************************************************************/
...
//         assert!(Subtensor::get_coldkey_balance(&source_wallet) < initial_balance - amount);
//         assert!(Subtensor::get_coldkey_balance(&dest_wallet) == amount);
//     });
// }
