...
impl<T: Config> Pallet<T> {
    ...
    pub fn update_pending_emissions() -> u64 {
        ...
        for (uid, neuron_stake) in <Stake<T> as IterableStorageMap<u64, u64>>::iter() {
    ...
    pub fn update_pending_emission_for_neuron(uid: u64, new_emission : u64) {
        PendingEmission::<T>::mutate(uid, |el| *el += new_emission);
    }
}
