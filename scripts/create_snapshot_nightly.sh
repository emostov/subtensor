...
SNAPSHOT_FILENAME=snapshot_$(date +"%m-%d-%Y")-nightly
...
# Add to ipfs
NEW_PIN=`ipfs add -Q -r /root/snapshots_nightly`
ipfs name publish /ipfs/$NEW_PIN

# Now let's build the new docker image
docker build -t subtensor . --platform linux/x86_64 --build-arg SNAPSHOT_DIR="snapshots_nightly" --build-arg SNAPSHOT_FILE=$SNAPSHOT_FILENAME  -f subtensor/Dockerfile
